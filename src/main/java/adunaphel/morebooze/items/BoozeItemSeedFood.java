...
public class BoozeItemSeedFood extends ItemFood implements IPlantable 
{
	...
	public BoozeItemSeedFood(int parHealAmount, float parSaturationModifier, Block parBlockPlant, Block parSoilBlock)
	{
	...
	public boolean onItemUse(ItemStack parItemStack, EntityPlayer parPlayer, World parWorld, int parX, int parY, int parZ, int par7, float par8, float par9, float par10)
	{
		...
		// not sure what this does, copied from potato
		if (par7 != 1)
		{
			return false;
		}
		...
		{
			// check that soil block can sustain plant
